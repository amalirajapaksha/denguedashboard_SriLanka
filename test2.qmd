...
#### **Last Updated at `r format(Sys.time()-86400, "%A %B %d, %Y")` (Latest local data available for: September 27, 2021)**
...
plotly::plot_ly(Vaccinedat,
  ...
  layout(yaxis = list(title = "Cumulative Doses Administrated"), autosize = F, width = 1650, height = 800)
...
The code behind the dashboard is available on [GitHub](https://github.com/thiyangt/coviddashboard.git).
...
The global data is taken from [coronavirus](https://github.com/RamiKrispin/coronavirus) package. The raw data in this package is being pulled from the Johns Hopkins University Center for Systems Science and Engineering (JHU CCSE) Coronavirus repository.
